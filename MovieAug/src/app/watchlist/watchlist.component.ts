...
export class WatchlistComponent implements OnInit {
  ...
  constructor(public restApi: RestApiService, private orderPipe: OrderPipe) {}
  ...
  loadMovie() {
    return this.restApi.getMovies().subscribe((data: {}) => {
      ...
      console.log("Test wathclist");

      this.watchlist = JSON.parse(localStorage.getItem("watchlist"));
      console.log(this.watchlist);

      this.watchlist.map(elem => {
        this.movies.map(movie => {
          if (elem.id === movie.id) {
            newsMovie.push(movie);
          }
        });
      });
      this.movies = newsMovie;
    });
  }
